        ...
        if(find(memorylist, reqseq)==int(memorylist.size()) || int(memorylist.size())==0)//没找到所要访问的页面
        {
            ...
            if(int(memorylist.size()) == blk){
                ...
                /*对页面标记进行排序，temp记录最久未被访问的页面*/
                 maxa = 0;
                for(g = 0; g < int(memorylist.size()); g++){
                    if(maxa < sign[g])
                        maxa = sign[g];
                }
                for(g = 0; g < int(memorylist.size()); g++){
    ...
    for(t = acclist.begin(); t != acclist.end(); )
    {
        ...
        if(find(memorylist, reqseq)==int(memorylist.size()) || int(memorylist.size())==0)//没找到所要访问的页面
        {
            ...
            if(int(memorylist.size()) == blk){
                ...
                for(i =k; i <= int(memorylist.size()); i++){
                    if(min == memorylist[i].access){
                        printf("memoeylist[%d].access=%d\n",i, memorylist[i].access);
                        k = i+1;
                        break;
                    }
                }
                if(k >= memorylist.size()){
                        k = 0;
                }
                if(i >= int(memorylist.size())){
                        i = 0;
                        i = i + 1;
                }
                memorylist[i].id = (*t).id;
                memorylist[i].access = (*t).access+1;
                memorylist[i].modify = (*t).modify;
                memorylist[i].status = (*t).status;
                t = acclist.erase(t);
            }else{
                /*在待访问页面序列中删除该页面，并增加其他页面相关的数据*/
                for(f=acclist.begin(); f!=acclist.end(); ++f){
    ...
    for(t = acclist.begin(); t != acclist.end(); )
    {
        ...
        if(find(memorylist,reqseq)==int(memorylist.size()) || int(memorylist.size())==0){//页面没在内存中
            ...
            if(int(memorylist.size()) == blk){//当内存中页面存满时的处理情况：使用循环指针k来进行判断：检查页面的访问位如果访问位为0，就将该页换出，为1，则将其置为0，将其留在内存中
                flag = 0;
                while(!flag){
                    for(int i = k; i < int(memorylist.size());i++){ //遍历整个内存：分两种情况，一种是访问位为0，一种是访问位为1
        ...
        if(find(memorylist, reqseq)==int(memorylist.size()) || int(memorylist.size())==0){//当内存中页面存满时的处理情况：使用循环指针k来进行判断,此时有页面在内存中和不在内存中两种情况
            ...
            if(int(memorylist.size()) == blk){
                ...
                while(!flag){
                    ...
                    if(k >= memorylist.size())
                        k = 0;
                    for(i = k; i < int(memorylist.size());i++){ //遍历整个内存：分两种情况，一种是访问位为0，一种是访问位为1
